...
0F8F7H: 	CALL WEIRD			; B = GET()
0F8FAH: 	MOV B,A
0F8FBH: 	MOV A,C
0F8FCH: 	CPI 3
0F8FEH: 	JC DF912			; IF C < 3 A ABORT
0F901H: 	CALL WEIRD
0F904H: 	MOV H,A				; H = GET()
0F905H: 	CALL WEIRD
0F908H: 	MOV L,A				; L = GET()
0F909H: 	CALL WEIRD
0F90CH: 	ANI 1				; IF GET()&1 D++
0F90EH: 	JZ DF912
0F911H: 	DAD D
0F912H: DF912:   ; 0F912H
0F912H: 	MOV A,B
0F913H: 	CPI 0C2H			
0F915H: 	JZ DF932			; IF B = 0xC2 THEN F932
0F918H: 	CPI 0D2H			
0F91AH: 	JZ DF93E			; IF B = 0xD2 THEN F932
0F91DH: 	CPI 0C6H
0F91FH: 	JZ JMP_HL           ; IF B = 0xC6 THEN JMP(HL)
0F922H: 	CPI 0C1H
0F924H: 	JC MONITOR_REENTER	; IF B < 0xC1 THEN ABORT
0F927H: 	CPI 0DBH
0F929H: 	JNC MONITOR_REENTER ; IF B >= 0xDB THEN ABORT
0F92CH: DF92C:   ; 0F92CH
0F92CH: 	CALL WEIRD
